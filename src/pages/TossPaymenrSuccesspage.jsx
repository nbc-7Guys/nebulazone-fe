...
const infoRow = {
    ...
    alignItems: "flex-start", // 위에 붙임
    marginBottom: 13,
    fontSize: 15,
    gap: 8, // label/value 사이 여유
};
...
function TossPaymentComponent() {
    ...
    const generateCustomerKey = () =>
        "customer_" + Math.random().toString(36).slice(2, 11);

    const generateOrderId = () =>
        "ORDER_" + Date.now() + "_" + Math.random().toString(36).slice(2, 7);
        ...
        try {
            ...
            await payment.requestPayment({
                ...
                successUrl:
                    ...
                    "/success?paymentKey={paymentKey}&orderId=" +
                    orderId +
                    "&amount=" +
                    amount,
...
function PaymentSuccess() {
    ...
    const handleConfirmPayment = async () => {
        ...
        try {
            const response = await fetch("http://localhost:8080/payments/confirm/toss", {
                method: "POST",
                headers: {
                ...
                body: JSON.stringify({
                    ...
                    point: Number(paymentInfo.amount),
                }),
            });
                ...
                {confirmResult && (
                    <div
                        style={{
                            ...
                            padding: 14,
                            borderRadius: 10,
                            background: confirmResult.success ? "#f1f8f2" : "#faeaea",
                            color: confirmResult.success ? "#18a857" : "#e24444",
                            fontSize: 13,
                            fontFamily: "monospace",
                        }}
                    >
...
const App = () => {
    ...
    useEffect(() => {
        if (window.location.pathname.includes("success")) setPage("success");
        else if (window.location.pathname.includes("fail")) setPage("fail");
